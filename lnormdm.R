...
#' @export
#'
#' @examples lnormdm(5, names = c("Control", "Treatment 1", "Treatment 2", "Treatment 3", "Treatment 4"), n= c(30,50,50,50,50), meanlog = c(3,4,3,2,3), sdlog= c(1,1,1,1,1))
#'
#'
#'
...
df2 <- purrr::map_dfr(df2, ~as.data.frame(t(.)))
...
P <- ggplot2::ggplot(simdata, mapping = aes(x=name,
                                            y=value,
                                            color = name)) +
