...
repeater_nbinom <- function(myoptimdm,myoptimparams,sims,pair,alt,ci){
  ...
  repeat{
    ...
    data_p <- tidyr::pivot_longer(data, cols = everything(), names_to = "groups",values_to = "samples")
    data_p$groups <- factor(data_p$groups)
    p <- rstatix::pairwise_t_test(data_p, samples ~ groups, pool.sd = FALSE,
    ...
    pwr <- length(which(p.values<ci))/(l*sims)
  }
  return(pwr)
}
